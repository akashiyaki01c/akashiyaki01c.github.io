...
import { DomWaker } from "./domwaker.ts";
...
function getHeaderTitle(header: unknown) {
	return (header as any)["title"];
}
/** Markdownファイルのヘッダ行からパンくずリストを取得する関数 */
function getHeaderPankuzu(header: unknown, isIndex: boolean): Pankuzu[] {
	const headerPankuzu = ((header as any)["pankuzu"]?.split(";")[1]) as string;
...
export function compileFromHtml(header: unknown, body: string, asideHeader: string, isIndex: boolean): string {
...
export function compileFromMarkdown(header: unknown, body: string, isIndex: boolean): string {
	const mdBody = render(body, options);
	const mdBody2 = DomWaker(mdBody);
	return compileFromHtml(header, mdBody2, getTocHtml(body), isIndex);
}
/** Markdown/HTMLファイルから、ヘッダ行とボディ行を分割して返す関数 */
function getHeaderAndBody(file: string): {header: unknown, body: string} {
	try {
		const output = extractToml(file);
		return {header: output.attrs, body: output.body}
	} catch {
...
export function compileFile(filePath: string) {
	const toFilePath = Path.resolve(import.meta.dirname || "", "../", filePath.replace("markdowns/", "files/").replace(".md", ".html"));
	...
	switch (Path.extname(filePath)) {
		...
		case ".css": {
			...
			const cssString = Deno.readTextFileSync(filePath);
			Deno.writeTextFileSync(toFilePath, cssString);
			break;
		}
		default: {
			fs.ensureDirSync(Path.dirname(toFilePath));
