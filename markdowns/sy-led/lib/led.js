...
class LedDisplay {
    #element = null;
    #inner_element = null;
    #inner_element_2 = null;

    constructor(id, imagePath, sizeX, sizeY, offsetX, offsetY, paddingX, paddingY, scale) {
        ...
        this.#element.innerHTML = `
            <div class="led-${id}-outer led-outer" style="width: ${sizeX * scale}px; height: ${sizeY * scale}px;">
                ...
                <div class="ami"></div>
                <div id="led-${id}-inner-2" class="${id}-inner led-inner-2" style="width: ${sizeX}px; height: ${sizeY}px; transform: scale(${scale});" ></div>
            </div>`;
        ...
        this.#inner_element_2 = document.getElementById(`led-${id}-inner-2`);
        this.#inner_element_2.style.backgroundImage = `url(${imagePath})`;
        this.#inner_element_2.style.backgroundPositionX = `-${offsetX}px`;
        this.#inner_element_2.style.backgroundPositionY = `-${offsetY}px`;
        this.#inner_element_2.style.width = `${sizeX}px`;
        this.#inner_element_2.style.height = `${sizeY}px`;
        this.#inner_element_2.style.imageRendering = `pixelated`;
    }

    update(x, y) {
        this.#inner_element.style.backgroundPositionX = `-${x * (this.sizeX + this.paddingX) + this.offsetX}px`;
        this.#inner_element.style.backgroundPositionY = `-${y * (this.sizeY + this.paddingY) + this.offsetY}px`;

        this.#inner_element_2.style.backgroundPositionX = `-${x * (this.sizeX + this.paddingX) + this.offsetX}px`;
        this.#inner_element_2.style.backgroundPositionY = `-${y * (this.sizeY + this.paddingY) + this.offsetY}px`;
    }
